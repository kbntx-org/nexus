...
repo's own `mise.toml`, and runs `mise install` before invoking Renovate — so `postUpgradeTasks` like
the pnpm lockfile fixup run with the exact toolchain versions this repo is pinned to, not whatever
ships in the Renovate image.
